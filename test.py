...
from network.Reconstruct_q import PGEDNN
...
def test_epoch(args, epoch, test_dataloader, model, logger_val, criterion, tb_logger):
    ...
    def get_padding(size):
        pad_total = (64 - size % 64) % 64
        return (pad_total // 2, pad_total - pad_total // 2)
    with torch.no_grad():
        for (d, filename) in tqdm(test_dataloader):
            d = d.to(device)
            d = d.unsqueeze(0)
            (orig_h, orig_w) = d.shape[-2:]
            (pad_top, pad_bottom) = get_padding(orig_h)
            (pad_left, pad_right) = get_padding(orig_w)
            d_padded = F.pad(d, (pad_left, pad_right, pad_top, pad_bottom), mode='reflect')
            (b, c, h_pad, w_pad) = d_padded.shape
            encryptor = ImageEncryptorDecryptor(b, c, h_pad, w_pad)
            final_encrypted = encryptor.encrypt(d_padded)
            final_decrypted = encryptor.decrypt(final_encrypted)
            torch.cuda.synchronize() if device.type == 'cuda' else None
            start_time = time.time()
            output = model(d_padded, encryptor)
            x_hat_padded = output['x_hat']
            x_hat = x_hat_padded[:, :, pad_top:pad_top + orig_h, pad_left:pad_left + orig_w]
            torch.cuda.synchronize() if device.type == 'cuda' else None
            end_time = time.time()
            time_meter.update((end_time - start_time) * 1000)
            output['x_hat'] = x_hat
            out_criterion = criterion(output, d)
            bpp_loss.update(out_criterion['bpp_loss'])
            loss.update(out_criterion['loss'])
            mse_loss.update(out_criterion['mse_loss'])
            ssim_value = ssim_metric(x_hat, d)
            ssim.update(ssim_value.item())
            save_dir = os.path.join('experiments', args.experiment, 'images')
            ori_dir = os.path.join(save_dir, 'ori')
            if not os.path.exists(ori_dir):
                os.makedirs(ori_dir)
            oriimg = torch2img(d)
            oriimg.save(os.path.join(ori_dir, filename))
            ...
            rec_img.save(os.path.join(rec_dir, filename))
            psnr.update(p)
            i = i + 1
    logger_val.info(f'Test epoch {epoch}: Average losses:\tLoss: {loss.avg:.3f} |\tMSE loss: {mse_loss.avg:.3f} |\tBpp loss: {bpp_loss.avg:.2f} |\tPSNR: {psnr.avg:.6f} |\tSSIM: {ssim.avg:.6f} |\tTime: {time_meter.avg:.3f}ms |')
...
def parse_args(argv):
    ...
    (parser.add_argument('--blocksize', default=16, type=int),)
...
def main(argv):
    ...
    test_transform = transforms.Compose([transforms.CenterCrop(512), transforms.ToTensor()])
    test_dataset = VariableSizeDataset(img_dir='/mnt/ssd2/gfy/Dataset/DIV2K_valid_HR', transform=test_transform)
    test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=True, num_workers=4, collate_fn=variable_collate, pin_memory=True)
    net = PGEDNN(in_channels=1, mid_channels=64, n_step=args.n_step, num_blocks=args.num_blocks, subrate=0.0625, blocksize=args.blocksize)
    net = net.to(device)
